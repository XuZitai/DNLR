...
class EncodeBlock(nn.Module):
    def __init__(self, in_channel, out_channel, flag):
        ...
        self.conv2 = nn.Conv3d(in_channel, out_channel, kernel_size=3, padding=1)
        ...
        self.nonlinear2 = nn.LeakyReLU(0.3)
    ...
    def forward(self, x, mask_in):
        ...
        out1 = self.conv1(x)
        mask_out = self.conv2(mask_in)
        out1 = out1 + (self.nonlinear1(out1) * self.sigmoid(mask_out))
        mask_out = torch.pow(self.nonlinear3(mask_out), 0.8)
        out1 = self.nonlinear2(out1)
        if self.flag:
            out1 = self.MaxPool(out1)
            mask_out = self.MaxPool(mask_out)
        else:
            out1 = out1
        return out1, mask_out

class DecodeBlock(nn.Module):
    def __init__(self, in_channel, mid_channel, out_channel, final_channel = 3, p = 0.7, flag = False):
        super(DecodeBlock, self).__init__()
        self.conv1 = nn.Conv3d(in_channel, mid_channel, kernel_size = 3, padding = 1)
        self.conv2 = nn.Conv3d(mid_channel, out_channel, kernel_size = 3, padding = 1)
        self.conv3 = nn.Conv3d(out_channel, final_channel, kernel_size = 3, padding = 1)
